          ...
          {COLUMN_KEYS.map((key, i) => {
            ...
            return (
              <Reveal key={key} delay={i * 0.08}>
                <div className={`rounded-2xl border border-line p-5 ${ordi ? "bg-card shadow-[var(--shadow-soft)]" : "bg-card/60"}`}>
                  ...
                  <dl className="mt-4 grid grid-cols-1 gap-3">
                    {COMPARISON.rows.map((row, r) => (
                      <div key={row.label} className={r > 0 ? "border-t border-dashed border-line pt-3" : ""}>
          ...
          <div className="card-ink overflow-hidden">
            <div className="grid grid-cols-[140px_repeat(3,minmax(0,1fr))]">
              ...
              {COMPARISON.headers.map((h, i) => (
                <div
                  key={h}
                  className={`border-b-2 border-l border-line px-5 py-4 font-headline text-xl leading-tight ${i === 2 ? "bg-highlight" : ""}`}
                >
                  {h}
                </div>
              ))}
              {COMPARISON.rows.map((row, r) => (
                <Fragment key={row.label}>
                  <div
                    className={`px-5 py-4 font-mono-label text-xs uppercase text-ink-2 ${r < COMPARISON.rows.length - 1 ? "border-b border-dashed border-line" : ""}`}
                  >
                    {row.label}
                  </div>
                  {COLUMN_KEYS.map((key) => (
                    <div
                      key={key}
                      className={`border-l border-line px-5 py-4 text-[15px] leading-relaxed ${
                        key === "ordi" ? "bg-highlight/35 text-ink" : "text-ink-2"
                      } ${r < COMPARISON.rows.length - 1 ? "border-b border-dashed border-b-line" : ""}`}
                    >
                      {row[key]}
                    </div>
                  ))}
