        ...
        <div className="grid grid-cols-1 gap-3">
          {FAQ_ITEMS.map((item, i) => (
            <Reveal key={item.q} delay={i * 0.04}>
              <details className="group rounded-2xl border border-line bg-card open:shadow-[var(--shadow-soft)]">
                <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 font-semibold [&::-webkit-details-marker]:hidden">
                  {item.q}
                  <span
                    className="grid grid-cols-1 size-8 shrink-0 place-items-center rounded-full border border-line bg-highlight text-lg leading-none transition-transform group-open:rotate-45"
                    aria-hidden="true"
                  >
                    +
                  </span>
