  ...
  return (
    <div className="flex flex-wrap items-start justify-center gap-7 sm:gap-10">
      ...
      <div className="receipt w-[250px] max-w-full rotate-[1.5deg] px-4 pt-4 pb-3 text-[11.5px] leading-relaxed">
        ...
        <div className="h-16" />
        <span className="stamp absolute bottom-4 right-4 text-[22px]">LUNAS</span>
      </div>
    </div>
  );
}
        ...
        <div className="mt-12 grid grid-cols-1 gap-7 md:grid-cols-3 md:gap-5">
          {PRICING_TIERS.map((tier, i) => (
            <Reveal key={tier.name} delay={i * 0.08} className="min-w-0">
              <div
                className={`relative flex h-full flex-col rounded-2xl border-2 border-ink bg-card p-6 ${
                  tier.highlight ? "shadow-[6px_6px_0_0_var(--color-ink)]" : ""
                }`}
              >
                {tier.ribbon && (
                  <span className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap rounded-full border-2 border-ink bg-ember px-4 py-1.5 font-mono-label text-[11px] font-semibold">
                    {tier.ribbon}
                  </span>
                )}
