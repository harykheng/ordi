    ...
    <div className="grid grid-cols-1 gap-4">
      <div className="grid grid-cols-2 gap-2.5">
        {[
          ...
          <div key={label} className="card-ink rounded-xl p-3">
            ...
            <p className="font-headline text-2xl leading-none">
      ...
      <PrintIn className="ticket-wrap w-[250px] max-w-full rotate-1">
        <div className="ticket px-5 py-3.5 font-mono-label text-[11.5px] leading-relaxed">
          <p className="mb-1.5 border-b-[1.5px] border-dashed border-ink/30 pb-1.5 font-headline text-base">Rekap produksi hari ini</p>
...
function Notif() {
  return (
    <div className="card-ink flex w-[270px] max-w-full items-center gap-3 rounded-2xl p-3">
      <span className="relative grid grid-cols-1 size-10 shrink-0 place-items-center rounded-full border-2 border-ink bg-highlight">
        ...
        <span className="absolute -right-1.5 -top-1.5 grid grid-cols-1 size-5 place-items-center rounded-full border-2 border-ink bg-ember font-mono-label text-[10px] font-bold">3</span>
      </span>
      <span>
    ...
    <div className="relative h-[124px] w-[260px] max-w-full" aria-hidden="true">
      {LABELS.map(([code, name], i) => (
        <motion.div
          ...
          className="absolute inset-x-0 flex justify-between rounded-[10px] border-2 border-ink bg-card px-3 py-2 font-mono-label text-xs font-semibold shadow-[0_3px_0_0_var(--color-ink)]"
  ...
  return (
    <div className="card-ink w-[280px] max-w-full p-3.5">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-sm font-bold">
        <span className="rounded-full border-2 border-ink bg-ink py-2 text-center text-paper">Konfirmasi</span>
        <span className="rounded-full border-2 border-ink py-2 text-center">Batalkan</span>
      </div>
...
function Recap() {
  return (
    <PrintIn className="receipt w-[270px] max-w-full -rotate-1 px-4 pt-4 pb-3 text-[11.5px] leading-relaxed">
      ...
      <p className="mt-3 rounded-full border-[1.5px] border-ink py-1.5 text-center font-semibold">Export CSV</p>
    </PrintIn>
  );
}
    ...
    <section id="dashboard" className="tear-top scroll-mt-20 bg-meja px-5 pt-24 pb-20 sm:pb-28">
      <div className="mx-auto max-w-6xl">
        ...
        <div ref={railRef} className="relative mt-14 grid grid-cols-1 gap-14 pl-9 sm:pl-12">
          <div className="absolute bottom-2 left-[9px] top-2 w-[3px] overflow-hidden rounded bg-ink/15 sm:left-[13px]" aria-hidden="true">
            <motion.div className="h-full w-full origin-top rounded bg-ink" style={{ scaleY: fill }} />
          </div>
          {OWNER_DAY.map((item) => {
            const Visual = VISUALS[item.visual];
            return (
              <Reveal key={item.time}>
                <div className="relative grid grid-cols-1 gap-5 md:grid-cols-[112px_minmax(0,1fr)_minmax(0,300px)] md:gap-8">
                  <span
                    className="absolute -left-[33px] top-1.5 size-[15px] rounded-full border-[2.5px] border-ink bg-ember sm:-left-[41px]"
                    aria-hidden="true"
                  />
                  <p className="flex items-center gap-2 self-start font-mono-label text-2xl font-bold leading-none">
                    {item.time >= "18:00" ? (
                      <svg viewBox="0 0 24 24" className="size-5 shrink-0" aria-hidden="true">
                        <path d="M20 14.5A8 8 0 1 1 9.5 4a6.5 6.5 0 0 0 10.5 10.5z" fill="#241A10" />
                      </svg>
                    ) : (
                      <svg viewBox="0 0 24 24" className="size-5 shrink-0" aria-hidden="true">
                        <circle cx="12" cy="12" r="5.5" fill="#FFC845" stroke="#241A10" strokeWidth="2" />
                        <path d="M12 1.5v3M12 19.5v3M1.5 12h3M19.5 12h3M4.6 4.6l2.1 2.1M17.3 17.3l2.1 2.1M4.6 19.4l2.1-2.1M17.3 6.7l2.1-2.1" stroke="#241A10" strokeWidth="2" strokeLinecap="round" />
                      </svg>
                    )}
                    {item.time}
                  </p>
                  <div>
                    <h3 className="font-headline text-[clamp(1.3rem,2.4vw,1.7rem)] leading-tight">{item.title}</h3>
        ...
        <Reveal>
          <div className="mt-16 flex flex-wrap items-center gap-2.5">
            ...
            {OWNER_EXTRAS.map((x) => (
              <span key={x} className="rounded-full border-[1.5px] border-ink bg-card px-3 py-1.5 text-[13px] font-semibold">
                {x}
              </span>
            ))}
          </div>
