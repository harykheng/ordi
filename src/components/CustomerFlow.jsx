...
function TierTag({ children }) {
  return (
    <span className="mt-3 inline-flex items-center gap-1.5 rounded-md border border-line bg-card px-2 py-1 font-mono-label text-[11px] font-semibold">
            ...
            {CUSTOMER_FLOW.map((step, i) => (
              ...
              >
                <div className={`transition-opacity duration-500 ${i === active ? "lg:opacity-100" : "lg:opacity-35"}`}>
                  <p className="grid size-10 place-items-center rounded-full bg-highlight font-headline text-lg text-ember-deep">{i + 1}</p>
                  <h3 className="font-headline mt-4 text-[clamp(1.5rem,2.8vw,2.1rem)] leading-tight">{step.title}</h3>
            ...
            <div className="sticky top-28 py-10">
              ...
              <div className="mt-6 flex items-center justify-center gap-2" aria-hidden="true">
                {CUSTOMER_FLOW.map((s, i) => (
                  <span
                    key={s.id}
                    className={`h-2 rounded-full border border-line transition-all duration-300 ${
