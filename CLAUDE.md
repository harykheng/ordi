...
- `landing-alternatif-3` — **this branch. Direction B, "Deretan Ruko"**:
  "own your shop instead of renting a stall" — striped awnings, hanging shop
  signs, a key tag for ownership, and an interactive street in the hero.
...
- **No full-bleed solid-color bands** (`bg-ink` or any strong color) as
  ...
  warm tones (`bg-paper` / `bg-meja`) alternating with scalloped edges are
  fine; strong weight goes into boards/cards, not bands.
...
- **Mockups are stylized boards and illustrations**, never realistic app
  screenshots in a phone frame. The hero catalog sits behind a shop window
  under an awning; the street is a flat SVG illustration.
...
## Design system — direction B (`src/index.css`)

A street of small shops on a warm morning. Token names are shared with the
other directions so the section components stay the same; only the values
and component classes change. `@theme`:

- `--color-paper` `#fff4e6` page (the hero adds a soft sun glow gradient)
- `--color-meja` `#fbe7cf` alternate sections; every meja section has a
  scalloped top edge via `.tear-top`, the next paper section sets
  `--tear-from: var(--color-meja)`
- `--color-card` `#fff` boards and cards, `--color-cream` `#fff8ee` light
  awning stripe
- `--color-ink` `#241a10` text, 2.5px outlines, **bottom** shadows
  (`0 5px 0` — painted-sign depth, not A's diagonal offset)
- `--color-ink-2` `#6a5646` secondary text (≥5.7:1 on paper and meja)
- `--color-ember` `#ff7a2f` awnings + primary fill (ink text on top, 6.5:1;
  white text fails)
- `--color-ember-deep` `#b04a12` orange text, emphasis (≥4.5:1 everywhere)
- `--color-pen` `#256b4d` "daun" green: step numbers, BUKA, LUNAS plate,
  checkmarks
- `--color-highlight` `#ffc845` lamp light, selected chips, number badges

Fonts (`index.html`): **Rammetto One** (`.font-headline`) for h1/h2, step
titles and signs only — it's wide and heavy, so sizes are smaller than in
direction A (h2 `clamp(1.9rem,3.8vw,2.8rem)`). Bricolage Grotesque was
dropped because it reads as AI-template. **Plus Jakarta Sans** body (same
face as the Ordi catalog), **IBM Plex Mono** for prices, codes, times.

Component classes: `.card-ink` (board), `.btn*` (rounded-xl, bottom shadow,
press down on hover), `.chip`, `.mark-hl` (orange + scalloped underline,
short phrases only), `.receipt` (pinned board; `.receipt-print` = board
drops in and swings), `.stamp` (green LUNAS plate), `.ticket`, `.hang`
(two strings above a board), `.awning-wrap` + `.awning` (striped, scalloped
hem, colour via `--awn`), `.sign-swing`, `.sign-flip`, `.tear-top`
(scallops), `.striped-closed`. Street SVG styles live under `.street`.

## Section structure (`App.jsx`)

```
Header        logo, anchors Fitur/Harga/Tanya, demo button
Hero          ownership headline; Shopfront (hanging sign flips + awning
              recolours per STORE_EXAMPLES, catalog plays inside the window),
              KeyTag ("kuncinya atas nama ..."), and Street: moving a cursor
              or finger along it switches shop lights on and flips TUTUP to
              BUKA (the Camemo-garden equivalent)
CustomerFlow  5 steps (#pelanggan), lamp-badge numbers. lg: sticky shop
              window (awning on top) swaps per step via IntersectionObserver;
              mobile: screen inline under each step
ModeSection   interactive harian vs PO calendar + daily quota stepper
OwnerDay      07:00-21:00 owner timeline (#dashboard), sun/moon per hour,
              scroll-filled rail
Comparison    table (md+) / stacked cards (mobile)
PricingTiers  3 tiers (awning over the Antar tier), "Semua paket sudah
              termasuk", "sewa lapak vs punya ruko" boards with a LUNAS plate
HowToStart    3 hanging process boards + "Biar nggak salah harap" notice
              board
...
`PrintIn` in `OwnerDay.jsx`). The street's lights use React state plus
per-shop timeouts; its `touch-action: pan-y` keeps vertical scrolling
working on phones while a horizontal drag lights the shops.
...
- `public/og-image.png` is rendered from an HTML mock of this direction's
  hero (sign, awning, key tag, street; 1200×630); regenerate it when the
